...
struct mydsp : public dsp {
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fRec1[0] = 0.5221894f * fRec1[3] + fTemp0 + 2.494956f * fRec1[1] - 2.0172658f * fRec1[2];
			int iTemp1 = 1 - iVec0[1];
			float fTemp2 = ((iTemp1) ? 0.0f : fConst1 + fRec3[1]);
			fRec3[0] = fTemp2 - std::floor(fTemp2);
			fVec2[IOTA0 & 1023] = static_cast<float>(input0[i0]);
			float fTemp3 = ((iTemp1) ? 0.0f : fConst7 + fRec5[1]);
			fRec5[0] = fTemp3 - std::floor(fTemp3);
			float fTemp4 = ((iTemp1) ? 0.0f : fConst8 + fRec6[1]);
			fRec6[0] = fTemp4 - std::floor(fTemp4);
			fRec4[0] = fVec2[(IOTA0 - static_cast<int>(std::min<float>(4096.0f, std::max<float>(0.0f, 1e+03f * (0.0006f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec6[0]), 65535))] + 0.00015f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec5[0]), 65535))] + 1.0f))))) & 1023] - fConst6 * (fConst5 * fRec4[2] + fConst3 * fRec4[1]);
			output0[i0] = static_cast<FAUSTFLOAT>(tanhf(1.5f * (fConst6 * (fRec4[2] + fRec4[0] + 2.0f * fRec4[1]) + 0.002f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec3[0]), 65535))] + 0.02f * (0.049922034f * fRec1[0] + 0.0506127f * fRec1[2] - (0.095993534f * fRec1[1] + 0.004408786f * fRec1[3])) + 0.2f * static_cast<float>(std::fabs(fTemp0) > 0.995f))));
			iVec0[1] = iVec0[0];
			iRec0[1] = iRec0[0];
